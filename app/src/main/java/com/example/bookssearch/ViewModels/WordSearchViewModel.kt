package com.example.bookssearch.ViewModels
...
class WordSearchViewModel(application: Application?) : AndroidViewModel(
    application!!
) {
    var apiResponseLiveData: LiveData<ApiResponse?>? = null
    var isfinished: LiveData<Boolean?>? = null
    private var searchBookRepo: SearchBookRepo? = null
    fun init() {
        searchBookRepo = SearchBookRepo()
        apiResponseLiveData = searchBookRepo?.apiResponseMutableLiveData
        isfinished = searchBookRepo?.isfinished
    }

    fun search(name: String?, author: String?) {
        searchBookRepo!!.search(name, author)
    }
}